...
export default function Layout({ headerStyle, footerStyle, breadcrumbTitle, children }: LayoutProps) {
  ...
  const [scrollDirection, setScrollDirection] = useState<string>("up"); // Add scroll direction state
  ...
  useEffect(() => {
    ...
    const handleScroll = (): void => {
      ...
      const scrollCheck: boolean = currentScroll > 100;
  ...
  return (
    <>
      ...
      {!headerStyle && (
        <Header1
          scroll={scroll}
          scrollDirection={scrollDirection} // Pass scroll direction to the header
      ...
      {headerStyle == 1 ? <Header1 scroll={scroll} scrollDirection={scrollDirection} isMobileMenu={isMobileMenu} handleMobileMenu={handleMobileMenu} isOffCanvas={isOffCanvas} handleOffCanvas={handleOffCanvas} /> : null}
